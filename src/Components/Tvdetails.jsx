...
function Tvdetails() {
  ...
  return info ? (
    <div
      style={{
        background: `linear-gradient(rgba(0,0,0,.1),rgba(0,0,0,.4),rgba(0,0,0,.8)), url(https://image.tmdb.org/t/p/original${info.detail.backdrop_path})`,
        backgroundSize: `cover`,
        backgroundPosition: `top[30%]`,
      }}
      className="w-screen h-[205vh] px-[10%] relative"
    >
      <nav className="w-full flex items-center h-[8vh] text-white text-xl gap-8">
        <Link>
          <i
            onClick={() => navigate(-1)}
            className="text-3xl font-semibold ri-arrow-left-line"
          ></i>
        </Link>
      ...
      <div className="w-full flex mt-[5%] gap-10">
        ...
        <div>
          ...
          <Link
            ...
            className="p-3 text-white rounded mt-2"
          >
            Watch Trailer
          </Link>
        </div>
      ...
      <div className="w-[80%]">
        <div className="mt-5 flex gap-4 items-center">
          {info.watchproviders && info.watchproviders.flatrate && (
            <h1 className="text-white text-xl">Flatrate Links:</h1>
          )}
          {info.watchproviders &&
        ...
        <div className="mt-5 flex gap-4 items-center">
          {info.watchproviders && info.watchproviders.rent && (
            <h1 className="text-white text-xl">Rent Links:</h1>
          )}
          {info.watchproviders &&
        ...
        <div className="mt-5 flex gap-4 items-center">
          {info.watchproviders && info.watchproviders.buy && (
            <h1 className="text-white text-xl">Buy Links:</h1>
          )}
          {info.watchproviders &&
      ...
      <h1 className="text-2xl font-semibold text-white mt-[5%] mb-2 ml-[1%]">
        Recommendations
      </h1>
      <HorizontalCards
        cards={
      ...
      />
      <hr className="h-[1.5px] border-none bg-zinc-500 mb-5 mt-3" />
      <h1 className="text-2xl font-semibold text-white mb-2 ml-[1%]">
        Seasons
      </h1>
      <div className="w-[100%] gap-5 flex overflow-y-hidden px-3 mb-5">
        {info.detail.seasons.map((s, i) => (
          <div className="min-w-[15%]">
            <img
              className="w-[26vh] shadow-[8px_17px_38px_2px_rgba(0,0,0,.5)] object-cover h-[40vh] "
              src={s.poster_path ? `https://image.tmdb.org/t/p/original/${
                s.poster_path}`: `https://st2.depositphotos.com/2102215/46681/v/450/depositphotos_466819550-stock-illustration-image-available-icon-missing-image.jpg`}
              alt=""
            />
            <h1 className="text-lg mb-3  text-zinc-300 mt-3">
              {s.name}
            </h1>
          </div>
        ))}
